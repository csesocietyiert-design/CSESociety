...
type Resource = { id: string; title: string; description?: string | null; resource_url: string; category: string; created_at: string };

export default function ResourcesPanel() {
  const user = useAuthStore((state) => state.user);
  const isAdmin = user?.role === 'admin';
  const [resources, setResources] = useState<Resource[]>([]);
  const [form, setForm] = useState({ title: '', description: '', resourceUrl: '', category: 'General' });
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setForm({ title: '', description: '', resourceUrl: '', category: 'General' });
    setShowForm(false);
  };

  const remove = async (id: string) => {
    const response = await fetch(`/api/resources/${id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user?.id }) });
  ...
  return <div className="mx-auto max-w-7xl space-y-6 pb-8">
    ...
    {showForm && <form onSubmit={submit} className="grid gap-4 rounded-2xl border border-slate-700/60 bg-slate-900/50 p-5 md:grid-cols-2"><input required value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Resource title" className="rounded-lg border border-slate-600 bg-slate-950/60 px-3 py-2.5 text-sm text-white" /><input value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })} placeholder="Category" className="rounded-lg border border-slate-600 bg-slate-950/60 px-3 py-2.5 text-sm text-white" /><input required type="url" value={form.resourceUrl} onChange={(event) => setForm({ ...form, resourceUrl: event.target.value })} placeholder="Resource link" className="rounded-lg border border-slate-600 bg-slate-950/60 px-3 py-2.5 text-sm text-white md:col-span-2" /><textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} placeholder="Description (optional)" rows={3} className="rounded-lg border border-slate-600 bg-slate-950/60 px-3 py-2.5 text-sm text-white md:col-span-2" /><button type="submit" className="rounded-lg bg-blue-500 px-4 py-2 text-sm font-semibold text-white md:col-span-2 md:justify-self-end">Save Resource</button></form>}
    {loading ? <p className="text-center text-slate-400">Loading resources...</p> : resources.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-700 p-10 text-center text-slate-400">No resources yet.</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{resources.map((resource) => <article key={resource.id} className="relative rounded-2xl border border-slate-700/70 bg-gradient-to-br from-slate-900 via-slate-900 to-blue-950/40 p-5"><div className="flex items-start justify-between gap-3"><span className="rounded-full border border-blue-500/30 bg-blue-500/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-blue-300">{resource.category}</span>{isAdmin && <button type="button" onClick={() => remove(resource.id)} className="text-xs text-red-300">Delete</button>}</div><h2 className="mt-5 text-xl font-bold text-white">{resource.title}</h2>{resource.description && <p className="mt-2 text-sm leading-6 text-slate-400">{resource.description}</p>}<a href={resource.resource_url} target="_blank" rel="noreferrer" className="mt-5 inline-flex text-sm font-medium text-teal-300">Open resource -&gt;</a></article>)}</div>}
  </div>;
}
