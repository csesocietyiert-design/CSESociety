...
const configs: Record<'secretary' | 'year_representative' | 'treasurer', DashboardConfig> = {
  secretary: {
    ...
    features: [
      ...
      { label: 'Resources', href: '/dashboard/reports', detail: 'Access society records', tone: 'blue' },
...
export default function RoleDashboard({ user, group }: { user: RoleDashboardUser; group: keyof typeof configs }) {
  ...
  const stats = [
    ...
    { label: 'Upcoming Events', value: eventsLoading ? '--' : String(events.length).padStart(2, '0'), tone: 'amber' },
    ...
    <div className="mx-auto max-w-7xl space-y-6 pb-8">
      <section className="relative overflow-hidden rounded-lg border border-slate-700/70 bg-gradient-to-br from-blue-600/20 via-slate-900/70 to-teal-700/10 px-6 py-7 shadow-2xl shadow-black/20 backdrop-blur-md sm:px-8">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-teal-300">{config.eyebrow}</p>
          ...
          <div className="border-t border-white/10 pt-4 text-right sm:border-l sm:border-t-0 sm:pl-6 sm:pt-0">
            ...
            <p className="mt-1 text-sm capitalize text-teal-300">{user.role.replaceAll('_', ' ')}</p>
          </div>
        </div>
      </section>
